...
export default function GameTemplate() {
  ...
  const [shuffleId, setShuffleId] = useState<number>(Date.now());
  ...
  useEffect(() => {
    const allCardsMatched =
      currentLevel && foundPairs.length === currentLevel.pairs * 2;
    if (allCardsMatched) {
      celebrateWin();
      handlePauseTimer();
    }
  }, [foundPairs, currentLevel]);
  ...
  const generateRandomCards = (level: GameLevel) => {
    ...
    return shuffleArray(pairPathImagesList);
  };
