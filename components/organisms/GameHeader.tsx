...
export function GameHeader({ level, foundPairCount,time }: Props) {
    ...
    return (
        <div className="flex max-sm:flex-col max-sm:items-start max-sm:pl-2 px-2 justify-between items-center rounded-md mb-8 gap-8 mt-4">
            <Button
                variant="outline"
                className="flex flex-row gap-3 cursor-pointer font-semibold rounded-2xl" onClick={() => router.push("/game/config")}>
                <ArrowLeftCircle />
